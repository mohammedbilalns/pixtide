...
export default function  AssetsGrid({assets}: AssetGridProps){
				...
				<div key={asset.id} className="border rounded-lg overflow-hidden bg-white shadow-sm hover:shadow transition-shadow">
					<div className="h-48 bg-slate-100 relative">
						...
						<div className="absolute top-2 right-2">
							<Badge className={asset.isApproved === "approved" ? "bg-teal-500" : asset.isApproved ==="rejected" ? "bg-red-500" : "bg-yellow-500" }  >
								{
									asset.isApproved === "approved" ? "Approved": 
										asset.isApproved === "rejected" ? "Rejected": 
											"Pending"
								}
							</Badge>
						</div>
